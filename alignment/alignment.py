...
def face_and_landmark_detect(image_original, face_detector, landmark_predictor_68, landmark_predictor_81, name, flag):
    ...
    if len(rects) == 0:
        ...
        if flag:
            ...
            landmarks = np.array([[p.x, p.y] for p in landmark_predictor_68(gray, rects[0]).parts()])
        # print("landmarks are: {}\n".format(landmarks))
    ...
    cv2.imwrite("./tmp/landmark_"+name+str(time.time())+".png", image)
...
def get_triangle_and_index(landmarks, image, name):
    ...
    cv2.imwrite('./tmp/'+'delaunay_'+str(time.time())+name, image)
    triangle_index = []
...
def load_image_and_morph(args):
    ...
    cv2.imwrite('./tmp/figure'+args.input_image.split('/')[-1].split('.')[0]+'__'+args.example_image.split('/')[-1].split('.')[0]+'.jpg', image_morph)
